...
namespace iLogger {
    ...
    string date_now() {
        ...
        snprintf(time_string, sizeof(time_string), "%04d-%02d-%02d", t.wYear, t.wMonth, t.wDay);
        return time_string;
    }
    ...
    string time_now() {
        ...
        snprintf(time_string, sizeof(time_string), "%04d-%02d-%02d %02d:%02d:%02d", t.wYear, t.wMonth, t.wDay, t.wHour, t.wMinute, t.wSecond);
        return time_string;
    }
...
#define SSCANF sscanf
    ...
    time_t gmtime2ctime(const string& gmt)
    {
        ...
        SSCANF(gmt.c_str(), "%3s, %2d %3s %4d %2d:%2d:%2d GMT", week, (unsigned)_countof(week), &date.tm_mday,
            month, (unsigned)_countof(month), &date.tm_year, &date.tm_hour, &date.tm_min, &date.tm_sec);
    ...
    string gmtime(time_t t)
    {
...
#if defined(U_OS_WINDOWS)
        if (::gmtime_s(&gmt, &t) != 0) return "Invalid time";
#else
        if (::gmtime_r(&t, &gmt) == nullptr) return "Invalid time";
#endif
        ...
        size_t len = std::strftime(buffer, sizeof(buffer), fmt, &gmt);
        return (len == 0) ? "Format failed" : std::string(buffer);
    }
    ...
    FILE* fopen_mkdirs(const string & path, const string & mode)
    {
        ...
        err = fopen_s(&f, path.c_str(), mode.c_str());
        return f;
    }
...
#define STRCPY(dst, src, size) strncpy(dst, src, size)
    ...
    bool pattern_match(const char* str, const char* matcher, bool igrnoe_case) {
        ...
        STRCPY(filter, matcher, sizeof(filter));
    ...
    string replace_extension(const std::string& path, const std::string& newExt) 
    {
