...
public:
    struct Job
    {
        ...
        MonopolyAllocator<ov::Tensor>::MonopolyDataPointer mono_tensor;
        std::shared_ptr<std::promise<Output>> pro;
    };

    virtual ~OVInferController()
    {
        stop();
    }
    ...
    bool startup(const StartParam& param)
    {
        ...
        start_param_ = param;
        worker_      = std::make_shared<std::thread>(&OVInferController::worker, this, std::ref(pro));
...
protected:
    ...
    std::shared_ptr<MonopolyAllocator<ov::Tensor>> tensor_allocator_;
};
