...
namespace TRT {
	...
	{
		int isCompressed;
		int ret = File7zIsCompressed2(onnxmodel, isCompressed);
		if (ret == 0 && isCompressed == 1) 
		{ 
			unsigned char* raw = nullptr;
			size_t rawSize = 0;
			if (File7zExtract(onnxmodel, &raw, &rawSize) == 0)
			{
				void* buf = std::malloc(rawSize);
				std::memcpy(buf, raw, rawSize);

				this->onnx_data_ = buf;
				this->onnx_data_size_ = rawSize;
				this->owns_data_ = true;
				this->type_ = ModelSourceType::OnnXData;
	...
	{
		int isCompressed;
		int ret = File7zIsCompressed2(onnxmodel.c_str(), isCompressed);
		if (ret == 0 && isCompressed == 1)
		{
			unsigned char* raw = nullptr;
			size_t rawSize = 0;
			if (File7zExtract(onnxmodel.c_str(), &raw, &rawSize) == 0)
			{
				void* buf = std::malloc(rawSize);
				std::memcpy(buf, raw, rawSize);
	...
	{
		if (owns_data_ && onnx_data_)
		{
			std::free(const_cast<void*>(onnx_data_));
		}
	}
		...
		bool next() 
		{
			...
			if (!tensor_)
			{
				...
				for (int i = 0; i < dims_.nbDims; ++i)
					dims_int[i] = static_cast<int>(dims_.d[i]);
				tensor_.reset(new Tensor(dims_.nbDims, dims_int));
		...
		const size_t maxWorkspaceSize) 
	{
		...
		for(int i = 0; i < net_num_layers; ++i)
		{
            ...
            //INFO("layer[%d] %s %s", i, name, layer_descript(layer).c_str());
		...
		for(int i = 0; i < net_num_input; ++i)
		{
			...
			nvinfer1::Dims minDims = input_dims;
			nvinfer1::Dims optDims = input_dims;
			nvinfer1::Dims maxDims = input_dims;
			...
			{
				for (int j = 2; j < inputsDimsSetup[i].dims().size(); ++j)
				{
					if (input_dims.d[j] == -1)
					{
						minDims.d[j] = inputsDimsSetup[i].dims()[j];
						optDims.d[j] = inputsDimsSetup[i].dims()[j];
						maxDims.d[j] = inputsDimsSetup[i].dims()[j];
					}
				}
			}
	...
	std::string get_file_fnv1a_hash(const std::string& file)
	{
		const std::uint64_t FNV_OFFSET_BASIS = 1469598103934665603ull;
		const std::uint64_t FNV_PRIME = 1099511628211ull;

		std::uint64_t hash = FNV_OFFSET_BASIS;

		int isCompressed = 0;
		int ret = File7zIsCompressed2(file.c_str(), isCompressed);
		if (ret == 0 && isCompressed == 1)
		{
			unsigned char* raw = nullptr;
			size_t rawSize = 0;
			if (File7zExtract(file.c_str(), &raw, &rawSize) == 0 && raw != nullptr && rawSize > 0)
			{
				for (size_t i = 0; i < rawSize; ++i)
				{
					hash ^= static_cast<unsigned char>(raw[i]);
					hash *= FNV_PRIME;
				}
				File7zFreeBuffer(raw);
			}
			else
			{
				...
				INFOE("Failed to extract compressed file: %s", file);
			}
		}
		else
		{
			...
			if (!ifs)
			{
				INFOE("Failed to open file: %s", file);
			}
