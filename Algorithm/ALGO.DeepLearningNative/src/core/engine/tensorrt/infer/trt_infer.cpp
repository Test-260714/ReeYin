...
namespace TRT {
	...
	public:
		...
		virtual int get_opt_batch_size() override;
	...
	static TRT::DataType convert_trt_datatype(nvinfer1::DataType dt){
		switch(dt){
			...
			case nvinfer1::DataType::kINT64: return TRT::DataType::Int64;
			default:
	...
	void InferImpl::build_engine_input_and_outputs_mapper() {
		...
		for (int i = 0; i < nbBindings; ++i) 
		{
			...
			auto newTensor = make_shared<Tensor>(dims.nbDims, dims_vec.data(), convert_trt_datatype(type));
	...
	void InferImpl::forward(bool sync) 
	{
		...
		for(int i = 0; i < context->engine_->getNbIOTensors(); ++i)
		{
			...
			if(context->engine_->getTensorIOMode(tensorName) == nvinfer1::TensorIOMode::kINPUT)
			{
				...
				{
                    INFOE("Failed to set input shape for %s", tensorName);
				}
			}
		}
		...
		{
			const char* tensorName = context->engine_->getIOTensorName(i);
			void* addr = orderdBlobs_[i]->gpu();

			if (!context->context_->setTensorAddress(tensorName, addr))
			{
				INFOF("Failed to set tensor address for %s", tensorName);
				return;
			}
		}
	...
	int InferImpl::get_max_batch_size()
	{
		...
		const char* tensorName = this->context_->engine_->getIOTensorName(0);
		auto dims = this->context_->context_->getTensorShape(tensorName);

		// 如果 batch 仍为 -1，则从 profile 中取最大 shape
		if (dims.d[0] < 0)
		{
			auto maxDims = this->context_->engine_->getProfileShape(tensorName, 0, nvinfer1::OptProfileSelector::kMAX);
			dims = maxDims;
		}

		return (dims.d[0] > 0) ? dims.d[0] : 1;
	}

	int InferImpl::get_opt_batch_size()
	{
		Assert(this->context_ != nullptr);
		...
		if (dims.d[0] < 0)
		{
			auto optDims = this->context_->engine_->getProfileShape(tensorName, 0, nvinfer1::OptProfileSelector::kOPT);
			dims = optDims;
		}
