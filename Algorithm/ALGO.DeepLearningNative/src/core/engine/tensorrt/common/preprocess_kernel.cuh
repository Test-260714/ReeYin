...
namespace CUDAKernel{
    ...
    void warp_affine_bilinear_and_normalize_plane_general(
        float* src, int src_line_size, int src_width, int src_height,
        float* dst, int dst_width, int dst_height,
        float* matrix_2_3, float const_value, const Norm& norm,
        cudaStream_t stream);

    void warp_affine_bilinear_and_normalize_image_mix_depth(
        float* src_image, int src_line_size, float* src_depth, int src_width, int src_height,
        float* dst, int dst_width, int dst_height,
        float* matrix_2_3, float const_value, const Norm& norm,
        cudaStream_t stream);

    void warp_affine_bilinear_and_normalize_depth_mix_mask(
        float* src_depth, float* src_mask, int src_width, int src_height,
        float* dst, int dst_width, int dst_height,
        float* matrix_2_3, float const_value, const Norm& norm,
        cudaStream_t stream);
    ...
    void clip_gray_value(float* src, int cols, int rows, float lower_limit, float upper_limit, float& min_val, float& max_val, cudaStream_t stream);

    void clip_gray_value_with_mask(float* src, float* src_mask, int cols, int rows, float lower_limit, float upper_limit, float& min_val, float& max_val, cudaStream_t stream);
