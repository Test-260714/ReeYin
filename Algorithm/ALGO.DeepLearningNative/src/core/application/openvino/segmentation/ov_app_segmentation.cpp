...
#include "ov_app_segmentation.h"
#include "ov_segmentation.h"
                          ...
                          Result** objInfo, int& objectNum)
    {
        try
        {
            ...
            if (_ov_param->splitImage)
            {
                ...
                MergePatches(tmp_bboxes_ptr, patchRois, validRois, -1, ModelType::MODEL_SEGMENTATION, bboxes);

                auto t = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - m_start).count();

                INFO("Segmentation model inference time: %.3f ms", t * 0.001);
