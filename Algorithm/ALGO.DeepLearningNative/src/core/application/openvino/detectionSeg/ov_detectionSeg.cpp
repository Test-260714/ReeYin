...
namespace DetectionSeg
{
    ...
    public:
        ...
        void yolo_mask_decode(float left, float top, float* mask_weights, float* mask_predict, 
        ...
        inline void yolo_post_process(float* pBboxArray, float* pMaskArray, ov::Shape bbox_output_shape, ov::Shape mask_output_shape, 
                                      int num_classes, float confidence_threshold, float* invert_affine_matrix, int max_objects, 
                                      std::vector<std::string> categories, BoxArray& objects)
        {
            objects.clear();
            objects.reserve(max_objects);

            int num_anchor = bbox_output_shape[1];

            int index = 0;
            for (int anchor_idx = 0; anchor_idx < num_anchor; anchor_idx++)
            {
                float* pitem = pBboxArray + (4 + num_classes + 32) * anchor_idx;
                ...
                // devode mask
                int mask_dim = mask_output_shape[1];
                float* mask_weights = pBboxArray + anchor_idx * bbox_output_shape[2] + num_classes + 4;

                float box_width = right - left;
                float box_height = bottom - top;
                float scale_to_predict_x = mask_output_shape[3] / (float)input_width_;
                float scale_to_predict_y = mask_output_shape[2] / (float)input_height_;
                ...
                if (mask_out_width > 0 && mask_out_height > 0)
                {
                    ...
                    float* mask_out = r.segmentation.floatData;

                    yolo_mask_decode(left * scale_to_predict_x, top * scale_to_predict_y, mask_weights,
                                     pMaskArray, mask_output_shape[3], mask_output_shape[2],
                                     mask_out, mask_dim, mask_out_width, mask_out_height);
                }
        ...
        inline void detr_post_process(int* label_array, float* box_array, float* score_array, float* mask_array, int mask_w, int mask_h,
                                      int num_bboxes, float confidence_threshold, int input_width, int input_height, float* invert_affine_matrix,
                                      int max_objects, std::vector<std::string> categories, BoxArray& objects)
        {
            ...
            memcpy(mask_affine_matrix, invert_affine_matrix, sizeof(mask_affine_matrix));
            mask_affine_matrix[0] = mask_affine_matrix[0] * (input_width_ / mask_w);
            mask_affine_matrix[4] = mask_affine_matrix[4] * (input_height_ / mask_h);
            ...
            for (int anchor_idx = 0; anchor_idx < num_bboxes; anchor_idx++)
            {
                ...
                float* pmask = mask_array + anchor_idx * NUM_MASK_ELEMENT;

                const int mask_count = mask_w * mask_h;
                ...
                r.segmentation = Seg(pmask, mask_w, mask_h, mask_affine_matrix, seg_threshold_);
        ...
        inline void yolo_process(int device_id, ov::CompiledModel compiled_model, std::promise<bool>& result)
        {
            ...
            auto bbox_output_port = compiled_model.output("bbox_outputs");
            auto mask_output_port = compiled_model.output("mask_outputs");
            ...
            while (get_job_and_wait(fetch_job))
            {
                ...
                const ov::Tensor& bbox_output_tensor = infer_request.get_tensor(bbox_output_port);
                const ov::Tensor& mask_output_tensor = infer_request.get_tensor(mask_output_port);

                ov::Shape bbox_output_shape = bbox_output_tensor.get_shape();
                ov::Shape mask_output_shape = mask_output_tensor.get_shape();
                int mask_dim = mask_output_shape[1];
                int num_classes = bbox_output_shape[2] - 4 - mask_dim;

#pragma warning(push)
#pragma warning(disable: 4996)
                const float* pBboxArray = bbox_output_tensor.data<float>();
                const float* pMaskArray = mask_output_tensor.data<float>();
#pragma warning(pop)

                auto& image_based_boxes = job.output;
                float* affine_matrix = job.additional.d2i;

                yolo_post_process((float*)pBboxArray, (float*)pMaskArray, bbox_output_shape, mask_output_shape, num_classes, confidence_threshold_,
        ...
        inline void detr_process(int device_id, ov::CompiledModel compiled_model, std::promise<bool>& result)
        {
            ...
            auto masks_port = compiled_model.output("masks");
            ...
            while (get_job_and_wait(fetch_job))
            {
                ...
                const ov::Tensor& masks_tensor = infer_request.get_tensor(masks_port);
                ...
                ov::Shape masks_tensor_shape = masks_tensor.get_shape();
                int mask_w = masks_tensor_shape[3];
                int mask_h = masks_tensor_shape[2];
                int NUM_MASK_ELEMENT = mask_w * mask_h;
...
#pragma warning(disable: 4996)
                ...
                const float* pMasksArray = masks_tensor.data<float>();
...
#pragma warning(pop)
                ...
                detr_post_process((int*)pLabelsArray, (float*)pBoxesArray, (float*)pScoresArray, (float*)pMasksArray, mask_w, mask_h,
                                  num_bboxes, confidence_threshold_, input_width_, input_height_, affine_matrix, max_objects_, categories_, image_based_boxes);
