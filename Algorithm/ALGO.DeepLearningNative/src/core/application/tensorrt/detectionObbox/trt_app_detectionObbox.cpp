...
#include "trt_app_detectionObbox.h"
                           ...
                           Result** objInfo, int& objectNum)
    {
        try
        {
            ...
            if (_trt_param->splitImage)
            {
                ...
                MergePatches(tmp_bboxes_ptr, patchRois, validRois, _trt_param->nmsThreshold, ModelType::MODEL_DETECTION_OBB, bboxes);

                auto t = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - m_start).count();

                INFO("Obb detection model inference time: %.3f ms", t * 0.001);
