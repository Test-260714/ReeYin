...
#include "trt_app_anomaly_detection.h"
...
namespace AnomalyDetection
{
    bool ConfigModel(std::shared_ptr<Param> param, std::shared_ptr<TRTInfer>& net)
    {
        ...
        bool sta = compile_trt_model(param, trtEngineFile, TRT::Mode::FP32);
        if (!sta)
        {
                           ...
                           Result** objInfo, int& objectNum)
    {
        try
        {
            ...
            if (imageData != nullptr && _trt_param->imageChannel > 0)
            {
                ...
                {
                    cv::Mat temp;
                    if (image.channels() == 1 && _trt_param->imageChannel == 3)
                    {
            ...
            {
                int patchWidth = std::max(1, static_cast<int>(std::round(_trt_param->inputWidth * _trt_param->accelerate)));
                int patchHeight = std::max(1, static_cast<int>(std::round(_trt_param->inputHeight * _trt_param->accelerate)));

                std::vector<ImagePatch> patches;
                GetImagePatches(image, cv::Mat(), patchWidth, patchHeight, _trt_param->overlapRate, patches);
                ...
                auto tmpResults = _trt_net->commits(imagePairs);
                MergeScorePatches(tmpResults, patchRois, image.size(), _trt_param->segThreshold,
                                  _trt_param->scoreTopRatio, results);
            ...
            else
            {
                ...
                auto m_start = std::chrono::high_resolution_clock::now();
                results = _trt_net->commit(imagePair).get();
                auto t = std::chrono::duration_cast<std::chrono::microseconds>(
                    std::chrono::high_resolution_clock::now() - m_start).count();
                INFO("Anomaly detection model inference time: %.3f ms", t * 0.001);
            }
