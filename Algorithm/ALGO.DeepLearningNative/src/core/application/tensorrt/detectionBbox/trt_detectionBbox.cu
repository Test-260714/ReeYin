...
namespace DetectionBbox
{
    ...
    static __global__ void decode_kernel_yolov8(float* predict, int num_bboxes, int num_classes, float confidence_threshold,
    ...
    static __global__ void decode_kernel_detr(int* labels, float* boxes, float* scores, int num_bboxes, float conf_thresh,
        int input_width, int input_height, float* invert_affine_matrix, float* parray, int MAX_IMAGE_BOXES)
    {
        int position = blockDim.x * blockIdx.x + threadIdx.x;
        if (position >= num_bboxes)
            return;
                                         ...
                                         float* invert_affine_matrix, float* parray, int max_objects, cudaStream_t stream) 
    {
        ...
        checkCudaKernel(decode_kernel_yolov8 << <grid, block, 0, stream >> > (predict, num_bboxes, num_classes, confidence_threshold, 
        ...
        float* parray, int max_objects, cudaStream_t stream)
    {
        ...
        checkCudaKernel(decode_kernel_detr << <grid, block, 0, stream >> > (label_array, box_array, score_array, num_bboxes, conf_thresh,
            input_width, input_height, invert_affine_matrix, parray, max_objects));
    }
