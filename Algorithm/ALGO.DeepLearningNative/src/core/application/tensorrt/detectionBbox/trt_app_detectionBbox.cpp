...
#include "trt_app_detectionBbox.h"
                           ...
                           Result** objInfo, int& objectNum)
    {
        try
        {
            ...
            if (_trt_param->splitImage)
            {
                ...
                MergePatches(tmp_bboxes_ptr, patchRois, validRois, _trt_param->nmsThreshold, MODEL_DETECTION_BBOX, bboxes);

                auto t = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - m_start).count();
                
                INFO("Detection model inference time: %.3f ms", t * 0.001);
