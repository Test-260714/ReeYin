...
namespace DetectionBbox
{
    ...
    public:
        ...
        inline void yolo_process(int gpuid, std::shared_ptr<TRT::Infer> engine, std::promise<bool>& result)
        {
            ...
            auto output = engine->tensor("bbox_outputs");
            ...
            while (get_jobs_and_wait(fetch_jobs, max_batch_size))
            {
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch)
                {
                    ...
                    yolo_bbox_decode_kernel_invoker(image_based_output, output->size(1), num_classes, confidence_threshold_,
        ...
        inline void detr_process(int gpuid, std::shared_ptr<TRT::Infer> engine, std::promise<bool>& result)
        {
            ...
            output_array_device.resize(opt_batch_size, 1 + MAX_IMAGE_BBOX * NUM_BOX_ELEMENT).to_gpu();
            ...
            while (get_jobs_and_wait(fetch_jobs, opt_batch_size))
            {
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch)
                {
                    ...
                    detr_bbox_decode_kernel_invoker(output_label_array, output_box_array, output_score_array, output_label_count,
                                                    confidence_threshold_, input_width_, input_height_, affine_matrix, output_array_ptr, MAX_IMAGE_BBOX, stream_);
