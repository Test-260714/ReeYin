...
//校正平面
HEAD void CallingConvention correctPlane(PointCloudNativeHandle* in_handle, float * normal, PointCloudNativeHandle* out_handle)
{
	auto* in_pc = PointCloudData(in_handle);
	auto* out_pc = PointCloudData(out_handle);
	if (in_pc == nullptr || out_pc == nullptr || normal == nullptr)
	{
		return;
	}
...
//sigam法则剔除阈值
HEAD void CallingConvention sigamFilter(PointCloudNativeHandle* in_handle, int sigam_thresh,
	PointCloudNativeHandle* out_handle)
{
	...
	out_pc->width = static_cast<std::uint32_t>(out_pc->points.size());
	out_pc->height = 1;
	InvalidatePointCloudBounds(out_handle);
...
HEAD void CallingConvention getRunoutPoints(PointCloudNativeHandle* in_handle, int num,
	PointCloudNativeHandle* out_handle)
{
	auto* in_pc = PointCloudData(in_handle);
	auto* out_pc = PointCloudData(out_handle);
	...
	if (EnsurePointCloudBounds(in_handle)) {
		minPt = in_handle->minPoint;
		maxPt = in_handle->maxPoint;
	}
...
//用于自己测试，不对外提供接口
HEAD float CallingConvention getRunoutPointsWithResult(PointCloudNativeHandle* in_handle, int num,
	PointCloudNativeHandle* out_handle)
{
	auto* in_pc = PointCloudData(in_handle);
	auto* out_pc = PointCloudData(out_handle);
...
HEAD double CallingConvention calculateRunout(PointCloudNativeHandle* in_handle, int * indices)
{
	auto* in_pc = PointCloudData(in_handle);
	if (in_pc == nullptr || indices == nullptr)
	{
		return 0.0;
	}
	...
	if (EnsurePointCloudBounds(in_handle)) {
		min_point = in_handle->minPoint;
		max_point = in_handle->maxPoint;
	}
	//找到Z值最小最大的两点索引
...
HEAD void CallingConvention copyPcBaseOnIndice(PointCloudNativeHandle* in_handle,
	PointCloudNativeHandle* out_handle)
{
	auto* in_pc = PointCloudData(in_handle);
	auto* out_pc = PointCloudData(out_handle);
	...
	*out_pc = *in_pc;
	CopyPointCloudBoundsIfValid(in_handle, out_handle);
}
	...
	PointCloudNativeHandle* out_handle)
{
	auto* in_pc = PointCloudData(in_handle);
	auto* out_pc = PointCloudData(out_handle);
	if (in_pc == nullptr || in_indices == nullptr || out_pc == nullptr)
	{
		return;
	}
	...
	out_pc->clear();
	InvalidatePointCloudBounds(out_handle);
	if (cluster_index < 0 || cluster_index >= static_cast<int>(in_indices->size()))
	{
		return;
	}

	const pcl::PointIndices& cluster = (*in_indices)[cluster_index];
	if (cluster.indices.empty())
	{
		return;
	}

	pcl::copyPointCloud(*in_pc, cluster, *out_pc);
	InvalidatePointCloudBounds(out_handle);
}
