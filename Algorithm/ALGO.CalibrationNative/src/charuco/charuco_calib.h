...
class CharucoCalibration final : public calib::boards::ICalibrationBoardCalibrator
{
public:
    /**
     * @brief 检测图像中的Charuco标定板角点
     * @param image 输入图像
     * @param boardParams Charuco标定板参数
    ...
    /**
     * @brief 计算Charuco点ID对应的物理坐标
     * @param boardParams Charuco标定板参数
     * @param pointIds Charuco点ID列表
...
private:
    /**
     * @brief 创建Charuco标定板
     * @param params Charuco标定板参数
     * @return Charuco标定板对象
     */
    static cv::aruco::CharucoBoard createCharucoBoard(const CalibrationBoardParams& params);
