...
{
    public class StateFlowRunnerViewModel: ObservableObject
    {
        private StateFlow? _stateMachine;
        ...
        protected StateFlowViewModel StateMachineViewModel { get; }
        ...
        public StateFlowRunnerViewModel(StateFlowViewModel stateMachineViewModel)
        {
        ...
        public async void Start()
        {
            ...
            _stateMachine = new StateFlow(StateMachineViewModel.States[0].Id, CreateStates(StateMachineViewModel.States), CreateBlackboard(StateMachineViewModel.Blackboard));
    ...
    public class HandleFlowRunnerViewModel : ObservableObject
    {
        private StateFlow? _stateMachine;
        private NodeViewModel? _activeState;
        private ConnectionViewModel? _activeTransition;
        private readonly DebugBlackboardDecorator _debugger = new DebugBlackboardDecorator();
        private readonly Blackboard _original = new Blackboard();

        protected NodifyEditorViewModel StateMachineViewModel { get; }
        ...
        public HandleFlowRunnerViewModel(NodifyEditorViewModel stateMachineViewModel)
        {
            StateMachineViewModel = stateMachineViewModel;
            _debugger.ValueChanged += OnBlackboardKeyValueChanged;
        }
        ...
        public async void Start()
        {
            ...
            _stateMachine = new StateFlow(StateMachineViewModel.Nodes[0].Id, CreateStates(StateMachineViewModel.Nodes), CreateBlackboard(StateMachineViewModel.Blackboard));
        ...
        private void HandleStateTransition(Guid from, Guid to)
        {
            ...
            _activeTransition = StateMachineViewModel.Transitions.FirstOrDefault(t => t.Source.Id == from);
            _activeState = StateMachineViewModel.Nodes.FirstOrDefault(st => st.Id == to);
        ...
        private IEnumerable<State> CreateStates(IEnumerable<NodeViewModel> states)
            => states.Select(s => new DebugStateDecorator(new State(s.Id, CreateTransitions(s), CreateAction(s.Action))));

        private IEnumerable<Transition> CreateTransitions(NodeViewModel state)
        {
            var transitions = StateMachineViewModel.Transitions.Where(t => t.Source == state).ToList();
            var result = new List<Transition>(transitions.Count);
