    ...
    public partial class IKapCamera: CameraBase
    {
        ...
        public override void ConnectDev()
        {
            try
            {
                ...
                bool isGigEOrUSB3 = string.Compare(g_devInfo.DeviceClass, "GigEVision") == 0 ||
                                    string.Compare(g_devInfo.DeviceClass, "USB3Vision") == 0;

                if (!isGigEOrUSB3)
                {
                    // 需要采集卡的相机类型
                    m_pBoard = new IntPtr(-1); // 标记需要采集卡但暂不打开
                    Console.WriteLine("此相机类型需要采集卡支持");
                }
                else
                {
                    m_pBoard = new IntPtr(-1); // 不需要采集卡
                }
