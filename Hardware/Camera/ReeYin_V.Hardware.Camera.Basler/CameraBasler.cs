    ...
    public class CameraBasler : CameraBase
    {
        ...
        public override void ConnectDev()
        {
            try
            {
                ...
                mCamera = new PylonCamera(SerialNo);
                mCamera.Open();
                ...
                Connected = true;
                ClearConnectionAlarms("Basler camera connected.");
            }
            catch (Exception ex)
            {
                //Logger.AddLog(Remarks + ":" + "Basler Connect:" + ex.Message, eMsgType.Error);
                Connected = false;
                ReportConnectionFailed(ex);
            }
        }
        ...
        public override bool CaptureImage(bool byHand)
        {
            ...
            catch (Exception ex)
            {
                ...
                ReportOperationFailed("CaptureImage", ex);
                if (TrigMode == eTrigMode.软触发)
                {
                    SignalWait.Set();
                }
        ...
        private IHardwareAlarmReporter GetAlarmReporter()
        {
            return PrismProvider.HardwareAlarmReporter;
        }

        private void ReportConnectionFailed(Exception exception)
        {
            try
            {
                GetAlarmReporter()?.ReportConnectionFailed(
                    BuildAlarmSource(),
                    BuildAlarmLocation(),
                    $"Basler camera connection failed: {exception.Message}",
                    exception,
                    BuildAlarmExtraData("ConnectDev"));
            }
            catch
            {
                // Alarm reporting must not interrupt camera connection flow.
            }
        }
        ...
        private void ReportDisconnected(string message)
        {
            try
            {
                GetAlarmReporter()?.ReportDisconnected(
                    BuildAlarmSource(),
                    BuildAlarmLocation(),
                    message,
                    null,
                    BuildAlarmExtraData("ConnectionLost"));
            }
            catch
            {
                // Alarm reporting must not interrupt camera event handling.
            }
        }

        private void ReportOperationFailed(string operation, Exception exception)
        {
            try
            {
                GetAlarmReporter()?.ReportOperationFailed(
                    BuildAlarmSource(),
                    BuildAlarmLocation(),
                    operation,
                    $"Basler camera operation failed: {exception.Message}",
                    exception,
                    BuildAlarmExtraData(operation));
            }
            catch
            {
                // Alarm reporting must not interrupt image acquisition flow.
            }
        }

        private void ClearConnectionAlarms(string note)
        {
            try
            {
                string source = BuildAlarmSource();
                string location = BuildAlarmLocation();
                GetAlarmReporter()?.Clear(HardwareAlarmCodes.ConnectionFailed, source, location, "System", note);
                GetAlarmReporter()?.Clear(HardwareAlarmCodes.Disconnected, source, location, "System", note);
            }
            catch
            {
                // Clearing a stale alarm must not make camera connection fail.
            }
        }

        private IDictionary<string, object?> BuildAlarmExtraData(string operation)
        {
        ...
        private string BuildAlarmSource()
        {
            ...
            if (!string.IsNullOrWhiteSpace(CameraNo))
            {
                return CameraNo.Trim();
            }

            return string.IsNullOrWhiteSpace(SerialNo) ? "BaslerCamera" : $"Basler-{SerialNo.Trim()}";
        }

        private string BuildAlarmLocation()
        {
            ...
            return string.IsNullOrWhiteSpace(CameraIP) ? "Unknown" : CameraIP.Trim();
        }
        ...
        private void OnConnectionLost(Object sender, EventArgs e)
        {
            ...
            ReportDisconnected("Basler camera connection lost.");
            DisConnectDev();
