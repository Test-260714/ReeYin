...
namespace Custom.WaferFlatnessMeasure.ViewModels
{
    ...
    public class SensorDataCollectionViewModel : DialogViewModelBase, IViewModuleParam
    {
        ...
        private void OpenDataAnalysisSourceFilterSetting(DataAnalysisDataSourceOption? dataSource)
        {
            dataSource ??= CurrentDataAnalysisDataSource;
            if (dataSource == null)
            {
                MessageBox.Show("请先选择一个数据源。", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            PrismProvider.DialogService.ShowDialog(
                "DataAnalysisSourceFilterSettingView",
