...
namespace Custom.WaferFlatnessMeasure.ViewModels
{
    public class WaferFlatnessConfigViewModel : DialogViewModelBase, IViewModuleParam
    {
        ...
        private ObservableCollection<EditableTrajectoryShape> _designedTrajectoryPreviewShapes =
            new ObservableCollection<EditableTrajectoryShape>();
        ...
        private readonly Dispatcher _uiDispatcher =
            PrismProvider.Dispatcher ?? Application.Current?.Dispatcher ?? Dispatcher.CurrentDispatcher;
        #endregion
        ...
        public ObservableCollection<EditableTrajectoryShape> DesignedTrajectoryPreviewShapes
        {
            get => _designedTrajectoryPreviewShapes;
            private set => SetProperty(ref _designedTrajectoryPreviewShapes, value ?? new ObservableCollection<EditableTrajectoryShape>());
        }

        private double _designedTrajectoryPreviewCoordinateStartX;
        public double DesignedTrajectoryPreviewCoordinateStartX
        {
            get => _designedTrajectoryPreviewCoordinateStartX;
            private set => SetProperty(ref _designedTrajectoryPreviewCoordinateStartX, value);
        }

        private double _designedTrajectoryPreviewCoordinateStartY;
        public double DesignedTrajectoryPreviewCoordinateStartY
        {
            get => _designedTrajectoryPreviewCoordinateStartY;
            private set => SetProperty(ref _designedTrajectoryPreviewCoordinateStartY, value);
        }

        private double _designedTrajectoryPreviewCoordinateEndX = 640d;
        public double DesignedTrajectoryPreviewCoordinateEndX
        {
            get => _designedTrajectoryPreviewCoordinateEndX;
            private set => SetProperty(ref _designedTrajectoryPreviewCoordinateEndX, value);
        }

        private double _designedTrajectoryPreviewCoordinateEndY = 480d;
        public double DesignedTrajectoryPreviewCoordinateEndY
        {
            get => _designedTrajectoryPreviewCoordinateEndY;
            private set => SetProperty(ref _designedTrajectoryPreviewCoordinateEndY, value);
        }

        private double _designedTrajectoryPreviewDefaultCenterX;
        public double DesignedTrajectoryPreviewDefaultCenterX
        {
            get => _designedTrajectoryPreviewDefaultCenterX;
            private set => SetProperty(ref _designedTrajectoryPreviewDefaultCenterX, value);
        }

        private double _designedTrajectoryPreviewDefaultCenterY;
        public double DesignedTrajectoryPreviewDefaultCenterY
        {
            get => _designedTrajectoryPreviewDefaultCenterY;
            private set => SetProperty(ref _designedTrajectoryPreviewDefaultCenterY, value);
        }
        ...
        public override void InitParam()
        {
            ...
            ModelParam.ApplyDesignedTrajectoryPlan();
            ...
            RefreshDesignedTrajectoryPreview();
            RefreshTrajectoryShapes();
        }
        ...
        public DelegateCommand EditDesignedTrajectoryCommand => new DelegateCommand(() =>
        {
            try
            {
                if (ModelParam == null)
                ...
                ModelParam.LoadKeyParam();
                DesignedTrajectoryPlan editorPlan = CreateEditorInitialPlan();
                var editorViewModel = new TrajectoryDesignerEditViewModel(
                    editorPlan,
                    ModelParam.CreateTrajectoryModel.CircleCenterX,
                    ModelParam.CreateTrajectoryModel.CircleCenterY,
                    ModelParam.GetSamplingSpacing());
                var window = new TrajectoryDesignerEditWindow
                {
                ...
                if (result != true || !editorViewModel.IsApplied)
                {
                    return;
                }

                ModelParam.ApplyDesignedTrajectoryPlan(editorViewModel.ResultPlan);
            ...
            {
                MessageBox.Show(ex.Message, "编辑轨迹失败", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        });
        ...
        private DesignedTrajectoryPlan CreateEditorInitialPlan()
        {
            DesignedTrajectoryPlan? existingPlan = ModelParam?.DesignedTrajectoryPlan;
            if (existingPlan != null &&
                ((existingPlan.Shapes?.Count ?? 0) > 0 || (existingPlan.RunSteps?.Count ?? 0) > 0))
            {
                return existingPlan.Clone();
            }

            var trajectoryModel = ModelParam?.CreateTrajectoryModel ?? new CreateTrajectoryModel();
            double centerX = trajectoryModel.CircleCenterX;
            double centerY = trajectoryModel.CircleCenterY;
            double radius = Math.Max(Math.Abs(trajectoryModel.CircleRadius), 50d);
            double halfRange = Math.Max(radius * 2d, 100d);

            return new DesignedTrajectoryPlan
            {
                CoordinateStartX = centerX - halfRange,
                CoordinateStartY = centerY - halfRange,
                CoordinateEndX = centerX + halfRange,
                CoordinateEndY = centerY + halfRange,
        ...
        {
            if (TryDispatchToUiThread(() => OnModelParamPropertyChanged(sender, e)))
            {
                return;
            }
            ...
            if (e.PropertyName == nameof(SensorMotionControlModel.DesignedTrajectoryPlan))
            {
                RefreshDesignedTrajectoryPreview();
                RefreshTrajectoryShapes();
                return;
        ...
        private bool TryDispatchToUiThread(Action action)
        {
        ...
        private void RefreshDesignedTrajectoryPreview()
        {
            DesignedTrajectoryPlan? plan = ModelParam?.DesignedTrajectoryPlan;
            if (plan?.Shapes?.Count > 0)
            {
                DesignedTrajectoryPreviewShapes = DesignedTrajectoryBuilder.ToEditableShapes(plan);
                SetDesignedTrajectoryPreviewWindow(
                    plan.CoordinateStartX,
                    plan.CoordinateStartY,
                    plan.CoordinateEndX,
                    plan.CoordinateEndY,
                    plan.DefaultCenterX,
                    plan.DefaultCenterY);
            ...
            var fallbackShapes = BuildPreviewShapesFromLocusInfos(ModelParam?.AllLocusInfo);
            DesignedTrajectoryPreviewShapes = fallbackShapes;
            SetDesignedTrajectoryPreviewWindowFromShapes(fallbackShapes);
        }

        private static ObservableCollection<EditableTrajectoryShape> BuildPreviewShapesFromLocusInfos(IEnumerable<LocusInfo>? locusInfos)
        {
            var shapes = new ObservableCollection<EditableTrajectoryShape>();
            foreach (LocusInfo locus in locusInfos ?? Enumerable.Empty<LocusInfo>())
            {
                if (locus == null ||
                ...
                double deltaX = locus.TargetX - locus.OriginX;
                double deltaY = locus.TargetY - locus.OriginY;
                double length = Math.Max(Math.Sqrt((deltaX * deltaX) + (deltaY * deltaY)), 1d);
                shapes.Add(new EditableTrajectoryShape
                {
                    Kind = TrajectoryShapeKind.Line,
                    X = (locus.OriginX + locus.TargetX) / 2d,
                    Y = (locus.OriginY + locus.TargetY) / 2d,
                    Width = length,
                    Height = 1d,
                    RotationAngle = Math.Atan2(deltaY, deltaX) * 180d / Math.PI
                });
            }
        ...
        private void SetDesignedTrajectoryPreviewWindowFromShapes(IReadOnlyCollection<EditableTrajectoryShape> shapes)
        {
            var model = ModelParam?.CreateTrajectoryModel;
            double centerX = model?.CircleCenterX ?? 0d;
            double centerY = model?.CircleCenterY ?? 0d;

            if (shapes == null || shapes.Count == 0)
            {
                double radius = Math.Max(Math.Abs(model?.CircleRadius ?? 50d), 50d);
                SetDesignedTrajectoryPreviewWindow(centerX - radius, centerY - radius, centerX + radius, centerY + radius, centerX, centerY);
                return;
            }

            double minX = shapes.Min(item => item.X);
            double maxX = shapes.Max(item => item.X);
            double minY = shapes.Min(item => item.Y);
            double maxY = shapes.Max(item => item.Y);

            foreach (EditableTrajectoryShape shape in shapes.Where(item => item.Kind == TrajectoryShapeKind.Line))
            {
                double halfLength = shape.Width / 2d;
                ...
                minX = Math.Min(minX, Math.Min(shape.X - offsetX, shape.X + offsetX));
                maxX = Math.Max(maxX, Math.Max(shape.X - offsetX, shape.X + offsetX));
                minY = Math.Min(minY, Math.Min(shape.Y - offsetY, shape.Y + offsetY));
                maxY = Math.Max(maxY, Math.Max(shape.Y - offsetY, shape.Y + offsetY));
            }

            double padding = Math.Max(Math.Max(maxX - minX, maxY - minY) * 0.08d, 10d);
            SetDesignedTrajectoryPreviewWindow(minX - padding, minY - padding, maxX + padding, maxY + padding, centerX, centerY);
        }

        private void SetDesignedTrajectoryPreviewWindow(
            double startX,
            double startY,
            double endX,
            double endY,
        ...
        {
            if (AreClose(startX, endX))
            {
                endX = startX + 1d;
            }

            if (AreClose(startY, endY))
            {
                endY = startY + 1d;
            }

            DesignedTrajectoryPreviewCoordinateStartX = startX;
            DesignedTrajectoryPreviewCoordinateStartY = startY;
            DesignedTrajectoryPreviewCoordinateEndX = endX;
            DesignedTrajectoryPreviewCoordinateEndY = endY;
            DesignedTrajectoryPreviewDefaultCenterX = defaultCenterX;
            DesignedTrajectoryPreviewDefaultCenterY = defaultCenterY;
        }
