...
namespace Custom.WaferFlatnessMeasure.Models
{
    ...
    public class GripperClampChannelModel : BindableBase
    {
        ...
        {
            get => _positionName;
            set => SetProperty(ref _positionName, string.IsNullOrWhiteSpace(value) ? string.Empty : value.Trim());
        }
        ...
        {
            get => ForwardJogAddress;
            set => ForwardJogAddress = value;
        }
        ...
        {
            get => ReverseJogAddress;
            set => ReverseJogAddress = value;
        }
        ...
        private string _forwardJogAddress = string.Empty;
        public string ForwardJogAddress
        {
            get => string.IsNullOrWhiteSpace(_forwardJogAddress) ? _clampCommandAddress : _forwardJogAddress;
            set
            {
                string normalized = NormalizeAddress(value);
                bool changed = SetProperty(ref _forwardJogAddress, normalized);
                if (_clampCommandAddress != normalized)
                {
                    _clampCommandAddress = normalized;
                    changed = true;
                }

                if (changed)
                {
                    RaisePropertyChanged(nameof(ClampCommandAddress));
                }
            }
        }

        private string _reverseJogAddress = string.Empty;
        public string ReverseJogAddress
        {
            get => string.IsNullOrWhiteSpace(_reverseJogAddress) ? _releaseCommandAddress : _reverseJogAddress;
            set
            {
                string normalized = NormalizeAddress(value);
                bool changed = SetProperty(ref _reverseJogAddress, normalized);
                if (_releaseCommandAddress != normalized)
                {
                    _releaseCommandAddress = normalized;
                    changed = true;
                }

                if (changed)
                {
                    RaisePropertyChanged(nameof(ReleaseCommandAddress));
                }
            }
        }

        private string _pressureAddress = string.Empty;
        public string PressureAddress
        {
            get => _pressureAddress;
            set => SetProperty(ref _pressureAddress, NormalizeAddress(value));
        }

        private double _pressureValue = double.NaN;
        [JsonIgnore]
        ...
        public void NormalizeConfiguredAddresses()
        {
            ...
            ForwardJogAddress = ForwardJogAddress;
            ReverseJogAddress = ReverseJogAddress;
            PressureAddress = PressureAddress;
    ...
    public class GripperClampControlModel : ModelParamBase
    {
        ...
        private static readonly string[] DefaultPositionNames = { "上", "下", "左", "右" };

        private static readonly string[] DefaultForwardJogAddresses = { "M3600", "M3610", "M3620", "M3630" };

        private static readonly string[] DefaultReverseJogAddresses = { "M3601", "M3611", "M3621", "M3631" };

        private static readonly string[] DefaultPressureAddresses = { "D220", "D222", "D224", "D226" };
        ...
        public Task<bool> StartJogAsync(int gripperIndex, string direction, CancellationToken cancellationToken = default)
        {
            return Task.Run(() =>
            {
                cancellationToken.ThrowIfCancellationRequested();
                ...
                EnsureGripperDefinitions();
                GripperClampChannelModel? gripper = GetGripper(gripperIndex);
                if (gripper == null || !gripper.IsEnabled)
                {
                    LastOperationSuccess = false;
                    LastOperationMessage = $"夹爪{gripperIndex}未启用或不存在";
                    return false;
                }

                bool isReverse = string.Equals(direction, "Reverse", StringComparison.OrdinalIgnoreCase);
                string commandAddress = isReverse ? gripper.ReverseJogAddress : gripper.ForwardJogAddress;
                string oppositeAddress = isReverse ? gripper.ForwardJogAddress : gripper.ReverseJogAddress;
                if (string.IsNullOrWhiteSpace(commandAddress))
                {
                    LastOperationSuccess = false;
                    LastOperationMessage = $"{gripper.Name}点动{(isReverse ? "反转" : "正转")}地址未配置";
                    return false;
                }

                if (!string.IsNullOrWhiteSpace(oppositeAddress))
                {
                    WriteBool(oppositeAddress, false);
                }

                bool success = WriteBool(commandAddress, true);
                if (success)
                {
                    gripper.ClampCommandValue = !isReverse;
                    gripper.ReleaseCommandValue = isReverse;
                    RefreshPressure(gripper);
                }

                LastOperationSuccess = success;
                LastOperationMessage = success
                    ? $"{gripper.Name}{gripper.PositionName}点动{(isReverse ? "反转" : "正转")}已启动"
                    : $"{gripper.Name}{gripper.PositionName}点动{(isReverse ? "反转" : "正转")}启动失败";
                LastOperationTime = DateTime.Now;
                RaiseStatusPropertiesChanged();
                RefreshOutputParamValues();
                return success;
        ...
        public Task<bool> StopJogAsync(int gripperIndex = 0, CancellationToken cancellationToken = default)
        {
            return Task.Run(() =>
            {
                cancellationToken.ThrowIfCancellationRequested();
                ...
                EnsureGripperDefinitions();
                IEnumerable<GripperClampChannelModel> grippers = gripperIndex > 0
                    ? Grippers.Where(item => item.Index == gripperIndex)
                    : GetActiveGrippers();
                ...
                foreach (GripperClampChannelModel gripper in grippers)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(gripper.ForwardJogAddress))
                    {
                        success &= WriteBool(gripper.ForwardJogAddress, false);
                    }

                    if (!string.IsNullOrWhiteSpace(gripper.ReverseJogAddress))
                    {
                        success &= WriteBool(gripper.ReverseJogAddress, false);
                    }
                ...
                LastOperationSuccess = success;
                LastOperationMessage = success ? "夹爪点动已停止" : "夹爪点动停止失败，请检查PLC日志";
                LastOperationTime = DateTime.Now;
                RaiseStatusPropertiesChanged();
                RefreshOutputParamValues();
                return success;
        ...
        public Task<bool> RefreshPressureAsync(int gripperIndex, CancellationToken cancellationToken = default)
        {
            return Task.Run(() =>
            {
                cancellationToken.ThrowIfCancellationRequested();
                ...
                bool success = RefreshPressure(gripper);
                LastOperationSuccess = success;
                LastOperationMessage = success ? $"{gripper.Name}压力读取完成" : $"{gripper.Name}压力读取失败";
                LastOperationTime = DateTime.Now;
                return success;
            }, cancellationToken);
        }

        public Task<bool> RefreshPressuresAsync(CancellationToken cancellationToken = default)
        {
            return Task.Run(() =>
            {
                cancellationToken.ThrowIfCancellationRequested();
                ...
                EnsureGripperDefinitions();
                bool success = true;
                foreach (GripperClampChannelModel gripper in GetActiveGrippers())
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    success &= RefreshPressure(gripper);
                }

                LastOperationSuccess = success;
                LastOperationMessage = success ? "四个夹爪压力读取完成" : "部分夹爪压力读取失败";
                LastOperationTime = DateTime.Now;
                return success;
            }, cancellationToken);
        }
        ...
        private bool RefreshPressure(GripperClampChannelModel gripper)
        {
            if (string.IsNullOrWhiteSpace(gripper.PressureAddress))
            {
                gripper.PressureValue = double.NaN;
                return false;
            }

            if (TryReadDouble(gripper.PressureAddress, out double pressure))
            {
                gripper.PressureValue = pressure;
                return true;
            }
        ...
        private bool TryReadDouble(string address, out double value)
        {
            value = double.NaN;
            if (_curPLC == null || string.IsNullOrWhiteSpace(address))
            {
            ...
            try
            {
                value = Convert.ToDouble(param.ParaValue, CultureInfo.InvariantCulture);
                return true;
            }
            catch (Exception ex)
            {
                Logs.LogError($"夹爪PLC压力读取值转换失败，地址：{param.PLCAddress}，值：{param.ParaValue}，异常：{ex.Message}");
                return false;
            }
        }
        ...
        private GripperClampChannelModel? GetGripper(int index)
        {
            EnsureGripperDefinitions();
            return Grippers.FirstOrDefault(item => item.Index == index);
        }
        ...
        private void EnsureGripperDefinitions()
        {
            ...
            for (int i = 0; i < _grippers.Count; i++)
            {
                ...
                if (string.IsNullOrWhiteSpace(gripper.PositionName))
                {
                    gripper.PositionName = DefaultPositionNames[i];
                }

                if (string.IsNullOrWhiteSpace(gripper.ForwardJogAddress))
                {
                    gripper.ForwardJogAddress = DefaultForwardJogAddresses[i];
                }

                if (string.IsNullOrWhiteSpace(gripper.ReverseJogAddress))
                {
                    gripper.ReverseJogAddress = DefaultReverseJogAddresses[i];
                }

                if (string.IsNullOrWhiteSpace(gripper.PressureAddress))
                {
                    gripper.PressureAddress = DefaultPressureAddresses[i];
                }
