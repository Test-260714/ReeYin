...
namespace Custom.WaferFlatnessMeasure.ViewModels
{
    public class WaferFlatnessConfigViewModel : DialogViewModelBase, IViewModuleParam
    {
        ...
        public DelegateCommand<string> GeneralCommand => new DelegateCommand<string>((order) =>
        {
            switch (order)
            {
                ...
                case "执行标定":
                    ...
                    ModelParam.ExecutePointMoving(ControlCard,true);
                    break;
                case "确认":
        ...
        private void RefreshTrajectoryShapes()
        {
            ...
            if (double.IsFinite(model.CircleCenterX) &&
            ...
            if (double.IsFinite(model.CircleRadius) && model.CircleRadius > 0)
            {
            ...
            if (!double.IsFinite(model.CircleCenterX) || !double.IsFinite(model.CircleCenterY))
            {
                return;
            }
