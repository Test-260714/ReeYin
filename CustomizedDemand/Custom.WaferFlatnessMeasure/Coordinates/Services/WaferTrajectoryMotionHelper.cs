...
namespace Custom.WaferFlatnessMeasure
{
    ...
    internal static class WaferTrajectoryMotionHelper
    {
        ...
        public static List<Dictionary<En_AxisNum, double>> BuildLineTrajectoryTargets(
            ...
            double z1Height)
        {
            ...
            foreach (var segment in segments)
            {
                targets.Add(new Dictionary<En_AxisNum, double>
                {
                    ...
                    { En_AxisNum.Z, zHeight },
                    { En_AxisNum.Z1, z1Height },
                    { En_AxisNum.Z2, 60 },
                });
        ...
        public static List<Dictionary<En_AxisNum, double>> BuildPointTrajectoryTargets(
            IEnumerable<LocusInfo> locusInfos,
            double zHeight,
            double z1Height)
        {
            var targets = new List<Dictionary<En_AxisNum, double>>();
        ...
        public static bool TryGetTrajectoryTargetSoftLimitViolation(
            ...
            out string violationMessage)
        {
            ...
            foreach (var target in targets)
            {
                foreach (var axisTarget in target)
                {
                    ...
                    if (axisConfig == null)
                    {
                        Logs.LogWarning($"Axis {axisTarget.Key} configuration is unavailable; soft-limit validation was skipped for this axis.");
                        continue;
                    }
                    ...
                    {
                        violationMessage =
                            $"Axis {axisTarget.Key} target {axisTarget.Value:F3} is outside " +
                            $"[{axisConfig.SoftLimitNegative:F3}, {axisConfig.SoftLimitPositive:F3}].";
                        return true;
                    }
                }
            }
