...
namespace Custom.MFDJC
{
    ...
    {
        public event EventHandler<DrawToolType> DrawingToolTypeChanged;
        ...
        public static readonly DependencyProperty BrushProperty = DependencyProperty.Register("Brush", typeof(SolidColorBrush), typeof(DrawingCanvas), new PropertyMetadata(Brushes.Red, OnBrushPropertyChanged));
        ...
        public static readonly DependencyProperty DrawingToolTypeProperty = DependencyProperty.Register("DrawingToolType", typeof(DrawToolType), typeof(DrawingCanvas), new PropertyMetadata(DrawToolType.Edit, OnDrawingToolTypePropertyChanged));
        ...
        public DrawToolType DrawingToolType
        {
            ...
            set
            {
                DrawingToolTypeChanged?.Invoke(this, value);
                this.SetValue(DrawingToolTypeProperty, value);
            }
        }
        ...
        protected override void OnMouseLeftButtonUp(MouseButtonEventArgs e)
        {
            ...
            if ((currentTime - _lastClickTime).TotalMilliseconds <= DoubleClickTime)
            {
                for (var i = workingDrawTools.Count - 1; i >= 0; i--)
                {
                    tool = workingDrawTools[i];

                    if (tool.TouchId == 0 && tool.CanDoubleClick && (e.Handled = tool.OnDoubleClick(point)))
                    {
        ...
        public event EventHandler<Point> OnMouseMoved;
        public event EventHandler<MouseEventArgs> OnMouseLeveled;
        public event EventHandler<MouseEventArgs> OnMouseEntered;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            // 获取鼠标在DrawingCanvasViewer的childBorder中的位置
            Point viewerPos = e.GetPosition(this);
            //坐标向下取整
            viewerPos.X = (int)viewerPos.X;
            viewerPos.Y = (int)viewerPos.Y;
            // 通知子控件鼠标移动事件
            OnMouseMoved?.Invoke(this, viewerPos);
        ...
        {
            drawTool.DrawToolEndedEvent += DrawingTool_DrawToolEnded;
        ...
        public void CencelWorkingDrawTool(IDrawTool drawTool)
        {
            var tempVisual = drawTool as Visual;
            base.RemoveLogicalChild(tempVisual);
            base.RemoveVisualChild(tempVisual);
            this.visuals.Remove(tempVisual);
            this.workingDrawTools.Remove(drawTool);
        }
        /// <summary>
        /// 转为图片
        ...
        #region 私有方法
        private void DrawingTool_DrawToolEnded(object? sender, DrawToolEventArgs e)
        {
        ...
        private IDrawTool CreateDrawingTool()
        {
            switch (DrawingToolType)
            {
                case DrawToolType.Pointer:
                    return new EditDrawTool(this);
                case DrawToolType.Edit:
                    return new EditDrawTool(this);
        ...
        private void UpdateCursor()
        {
            switch (this.DrawingToolType)
            {
                ...
                case DrawToolType.Text:
                    ...
                    break;
                default:
                    if ((squareCursor && cursorLength == StrokeThickness && cursorBrush == Brush) || Double.IsNaN(StrokeThickness))
                        break;
                    var w = (UInt32)Math.Max(1, StrokeThickness);
                    var h = (UInt32)Math.Max(1, StrokeThickness);
                    var border = (UInt32)Math.Max(1, 2 * 1);
        ...
        private const int DoubleClickTime = 200; // 双击的最大时间间隔（毫秒）
