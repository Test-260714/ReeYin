...
namespace Custom.WaferFlatnessMeasure.Models
{
    ...
    public partial class SensorDataCollectionModel : ModelParamBase
    {
        ...
        [JsonIgnore]
        private SensorTrajectoryDataPackage _collectionDataPackage = new SensorTrajectoryDataPackage();
        #endregion
        ...
        [JsonIgnore]
        [OutputParam("CollectionDataPackage", "最近一次传感器与运动轨迹采集数据包")]
        public SensorTrajectoryDataPackage CollectionDataPackage
        {
            get { return _collectionDataPackage; }
            set
            {
                _collectionDataPackage = value ?? new SensorTrajectoryDataPackage();
                RaisePropertyChanged();
            }
        }
        ...
        public int MedianFilterWindowSize
        {
            ...
            set
            {
                _medianFilterWindowSize = NormalizeFilterWindowSize(value);
                RaisePropertyChanged();
                UpdateCollectionDataPackage();
            }
        }
        ...
        public int SmoothFilterWindowSize
        {
            ...
            set
            {
                _smoothFilterWindowSize = NormalizeFilterWindowSize(value);
                RaisePropertyChanged();
                UpdateCollectionDataPackage();
            }
        }

        private bool _isFilterOriginalDataValues;
        /// <summary>
        /// 是否同步滤波 OriginalDataValues 中除核心上下表面外的其它原始数值。
        /// </summary>
        public bool IsFilterOriginalDataValues
        {
            ...
            set
            {
                _isFilterOriginalDataValues = value;
                RaisePropertyChanged();
                UpdateCollectionDataPackage();
            }
        }
        ...
        public string LastPreDatasCsvPath
        {
            ...
            set
            {
                _lastPreDatasCsvPath = value ?? string.Empty;
                RaisePropertyChanged();
                UpdateCollectionDataPackage();
            }
        }
        ...
        public string LastCalibrationWaferDataCsvPath
        {
            ...
            set
            {
                _lastCalibrationWaferDataCsvPath = value ?? string.Empty;
                RaisePropertyChanged();
                UpdateCollectionDataPackage();
            }
        }
        ...
        public void TrrigerStopCollect(string order)
        {
            ...
            try
            {
                ...
                UpdatePreDatas(ApplyMeasurementFilters(BuildPreprocessDatas(DataCollect)));
                if (IsPoint)
                {
        ...
        private void ExecuteCsvModule()
        {
            ...
            UpdatePreDatas(ApplyMeasurementFilters(csvPreDatas));
        ...
        private List<PreprocessDatasetModel> ApplyMeasurementFilters(IEnumerable<PreprocessDatasetModel>? preDatas)
        {
            MeasurementDataFilterOptions filterOptions = BuildMeasurementDataFilterOptions();
            if (!filterOptions.MedianFilterEnabled && !filterOptions.SmoothFilterEnabled)
            {
                return PreprocessDatasetModel.Clone(preDatas);
            }

            List<PreprocessDatasetModel> filteredPreDatas = PreprocessDatasetFilter.Apply(preDatas, filterOptions);
            Logs.LogInfo(
                $"采集数据滤波完成：中值滤波={filterOptions.MedianFilterEnabled}({filterOptions.MedianFilterWindowSize})，平滑滤波={filterOptions.SmoothFilterEnabled}({filterOptions.SmoothFilterWindowSize})，原始数据扩展字段={filterOptions.FilterOriginalDataValues}。");
            return filteredPreDatas;
        }

        private MeasurementDataFilterOptions BuildMeasurementDataFilterOptions()
        {
            return new MeasurementDataFilterOptions
            {
                MedianFilterEnabled = IsMedianFilterEnabled,
                MedianFilterWindowSize = NormalizeFilterWindowSize(MedianFilterWindowSize),
                SmoothFilterEnabled = IsSmoothFilterEnabled,
                SmoothFilterWindowSize = NormalizeFilterWindowSize(SmoothFilterWindowSize),
                FilterUpSurface = true,
                FilterDownSurface = true,
                FilterThickness = true,
        ...
        private static int NormalizeFilterWindowSize(int windowSize)
        {
        ...
        private void UpdateCollectionDataPackage()
        {
            try
            {
                List<PointCollectionStepInfo> pointCollectionSteps = _pointCollectionSteps
                    .Where(step => step != null)
                    .Select(step => step.Clone())
                    .ToList();
                ...
                int lineSegmentCsvExpectedCount;
                lock (_lineSegmentCsvSessionLock)
                {
                    lineSegmentCsvSessionDirectory = _lineSegmentCsvSessionDirectory;
                    lineSegmentCsvIndex = _lineSegmentCsvIndex;
                    lineSegmentCsvExpectedCount = _lineSegmentCsvExpectedCount;
                }

                List<LineSegmentStartPositionInfo> lineSegmentStartPositions;
                lock (_lineSegmentStartPositionsLock)
                {
                    lineSegmentStartPositions = _lineSegmentStartPositions
                        .Where(position => position != null)
                        .Select(position => position.Clone())
                        .ToList();
                }

                CollectionDataPackage = SensorTrajectoryDataPackage.Create(
                    IsPoint,
                    PreDatas,
                    pointCollectionSteps,
                    lineSegmentStartPositions,
                    lineSegmentCsvSessionDirectory,
                    lineSegmentCsvIndex,
                    lineSegmentCsvExpectedCount,
                    ResolveCurrentSourceCsvPath(),
                    LastPreDatasCsvPath,
                    LastCalibrationWaferDataCsvPath,
                    SltModel?.NickName ?? SltModelName,
                    UpSurfaceOriginalDataValueName,
                    DownSurfaceOriginalDataValueName,
                    BuildMeasurementDataFilterOptions());
            }
            catch (Exception ex)
            {
                Logs.LogError($"刷新传感器与轨迹采集数据包失败：{ex.Message}{Environment.NewLine}{ex}");
            }
        }

        private string ResolveCurrentSourceCsvPath()
        {
