...
namespace Custom.WaferFlatnessMeasure
{
    ...
    public class CreateTrajectoryModel : BindableBase
    {
        ...
        private double _rectangleStartX = -50;
        private double _rectangleStartY = -50;
        private double _rectangleEndX = 50;
        private double _rectangleEndY = 50;
        private double _horizontalLineSpacing = 10;
        ...
        private CalibrationWaferMeasurementMode _calibrationWaferMeasurementMode = CalibrationWaferMeasurementMode.不测量;
        ...
        public double RectangleStartX
        {
            get => _rectangleStartX;
            set
            {
                if (SetProperty(ref _rectangleStartX, value))
                {
                    RaisePropertyChanged(nameof(CircleGeneratorSummaryText));
                }
            }
        ...
        public double RectangleStartY
        {
            get => _rectangleStartY;
            set
            {
                if (SetProperty(ref _rectangleStartY, value))
                {
                    RaisePropertyChanged(nameof(CircleGeneratorSummaryText));
                }
            }
        ...
        public double RectangleEndX
        {
            get => _rectangleEndX;
            set
            {
                if (SetProperty(ref _rectangleEndX, value))
                {
                    RaisePropertyChanged(nameof(CircleGeneratorSummaryText));
                }
            }
        ...
        public double RectangleEndY
        {
            get => _rectangleEndY;
            set
            {
                if (SetProperty(ref _rectangleEndY, value))
                {
                    RaisePropertyChanged(nameof(CircleGeneratorSummaryText));
                }
            }
        ...
        {
            get => _horizontalLineSpacing;
            set
            {
                double normalizedValue = double.IsFinite(value) ? Math.Max(value, 0.001) : 0.001;
                if (SetProperty(ref _horizontalLineSpacing, normalizedValue))
                {
                    RaisePropertyChanged(nameof(CircleGeneratorSummaryText));
                }
            }
        ...
        public CircleTrajectoryGenerateMode CircleGenerateMode
        {
            ...
            set
            {
                if (SetProperty(ref _circleGenerateMode, value))
                {
                    ...
                    RaisePropertyChanged(nameof(IsRectangleHorizontalLineEditable));
                    ...
                    RaisePropertyChanged(nameof(IsCalibrationWaferMeasurementModeVisible));
                    RaisePropertyChanged(nameof(IsCalibrationWaferMeasurementActive));
                    RaisePropertyChanged(nameof(SelectedCircleGenerateModeTabIndex));
                }
            }
        }

        public CalibrationWaferMeasurementMode CalibrationWaferMeasurementMode
        {
            get => _calibrationWaferMeasurementMode;
            set
            {
                if (SetProperty(ref _calibrationWaferMeasurementMode, value))
                {
                    RaisePropertyChanged(nameof(CircleGeneratorSummaryText));
                    RaisePropertyChanged(nameof(IsCalibrationWaferMeasurementActive));
                }
        ...
        [JsonIgnore]
        public bool IsRectangleHorizontalLineEditable =>
            CircleGenerateMode == CircleTrajectoryGenerateMode.等间距水平线;
        ...
        [JsonIgnore]
        public bool IsCalibrationWaferMeasurementModeVisible =>
            CircleGenerateMode == CircleTrajectoryGenerateMode.内切正方形点;

        [JsonIgnore]
        public bool IsCalibrationWaferMeasurementActive =>
            IsCalibrationWaferMeasurementModeVisible &&
            CalibrationWaferMeasurementMode != CalibrationWaferMeasurementMode.不测量;
        ...
        public string CircleGeneratorSummaryText
        {
            get
            {
                if (IsRectangleHorizontalLineEditable)
                {
                    return GetRectangleHorizontalLineSummaryText();
                }
                ...
                return CircleGenerateMode switch
                {
                    ...
                    CircleTrajectoryGenerateMode.内切正方形点 =>
                        $"将在边长 {RoundCoordinate(GetInscribedSquareSideLength())} 的内切正方形内按间距 {RoundCoordinate(PointSpacing)} 生成 {CalculateInscribedSquarePointCount()} 个点。{GetCalibrationWaferMeasurementSummary()}",
        ...
        private List<LocusInfo> BuildCircleLocusInfos(double sensorInterval = 0)
        {
            ...
            return CircleGenerateMode switch
            {
                ...
                CircleTrajectoryGenerateMode.等间距水平线 => BuildRectangleHorizontalLineLocusInfos(sensorInterval),
        ...
        /// 按矩形对角线起终点和行间距生成水平线。
        /// </summary>
        private List<LocusInfo> BuildRectangleHorizontalLineLocusInfos(double sensorInterval = 0)
        {
            ValidateRectangleHorizontalLineParams();
            double minX = Math.Min(RectangleStartX, RectangleEndX);
            double maxX = Math.Max(RectangleStartX, RectangleEndX);
            double minY = Math.Min(RectangleStartY, RectangleEndY);
            double maxY = Math.Max(RectangleStartY, RectangleEndY);
            double spacing = NormalizeHorizontalLineSpacing();
            var yPositions = GenerateRectangleHorizontalLineYPositions(minY, maxY, spacing);
            var locusInfos = new List<LocusInfo>(yPositions.Count);

            foreach (double y in yPositions)
            {
                double originX = RoundCoordinate(minX);
                double originY = RoundCoordinate(y);
                double targetX = RoundCoordinate(maxX);
                double targetY = RoundCoordinate(y);
        ...
        private static List<double> GenerateRectangleHorizontalLineYPositions(double minY, double maxY, double spacing)
        {
            ...
            if (yPositions.Count == 0 || Math.Abs(yPositions[^1] - maxY) > 1e-9)
            {
                yPositions.Add(maxY);
            }
        ...
        private void ValidateRectangleHorizontalLineParams()
        {
            if (!double.IsFinite(RectangleStartX) ||
                !double.IsFinite(RectangleStartY) ||
                !double.IsFinite(RectangleEndX) ||
                !double.IsFinite(RectangleEndY))
            {
                throw new InvalidOperationException("矩形对角线坐标必须是有效数值。");
            }

            if (Math.Abs(RectangleEndX - RectangleStartX) < 1e-9 ||
                Math.Abs(RectangleEndY - RectangleStartY) < 1e-9)
            {
                throw new InvalidOperationException("矩形宽度和高度必须大于 0。");
            }

            if (!double.IsFinite(HorizontalLineSpacing) || HorizontalLineSpacing <= 0)
            {
                throw new InvalidOperationException("水平线间距必须大于 0。");
            }
        }
        ...
        private double NormalizeHorizontalLineSpacing()
        {
            ValidateRectangleHorizontalLineParams();
        ...
        private string GetRectangleHorizontalLineSummaryText()
        {
            if (!double.IsFinite(RectangleStartX) ||
                !double.IsFinite(RectangleStartY) ||
                !double.IsFinite(RectangleEndX) ||
                !double.IsFinite(RectangleEndY))
            {
                return "请输入有效的矩形对角线坐标。";
            }

            if (Math.Abs(RectangleEndX - RectangleStartX) < 1e-9 ||
                Math.Abs(RectangleEndY - RectangleStartY) < 1e-9)
            {
                return "矩形宽度和高度必须大于 0。";
            }

            if (!double.IsFinite(HorizontalLineSpacing) || HorizontalLineSpacing <= 0)
            {
                return "请输入大于 0 的水平线间距。";
            }

            double minY = Math.Min(RectangleStartY, RectangleEndY);
            double maxY = Math.Max(RectangleStartY, RectangleEndY);
            int lineCount = GenerateRectangleHorizontalLineYPositions(minY, maxY, HorizontalLineSpacing).Count;

            return $"将在矩形对角线 ({RoundCoordinate(RectangleStartX)}, {RoundCoordinate(RectangleStartY)}) 到 ({RoundCoordinate(RectangleEndX)}, {RoundCoordinate(RectangleEndY)}) 内按间距 {RoundCoordinate(HorizontalLineSpacing)} 生成 {lineCount} 条等间距水平线。";
        }

        private string GetCalibrationWaferMeasurementSummary()
        {
            return CalibrationWaferMeasurementMode switch
            {
                CalibrationWaferMeasurementMode.每行开始前 => "每行开始前会先测量一次标准片位置。",
                CalibrationWaferMeasurementMode.每点测量前 => "每个晶圆点测量前会先测量一次标准片位置。",
                _ => string.Empty
            };
        }
    ...
    [Serializable]
    public enum CalibrationWaferMeasurementMode
    {
        不测量,
        每行开始前,
        每点测量前
