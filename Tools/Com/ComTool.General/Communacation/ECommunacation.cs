using ComTool.General.Communacation.Tool;
...
namespace ComTool.General.Communacation
{
    ...
    [Serializable]
    public class ECommunacation : BindableBase
    {
        #region Fields
        public Guid guid { get; set; } = Guid.NewGuid(); //唯一标识符
        #endregion
                ...
                if (_SendCommand == null)
                {
                    _SendCommand = new DelegateCommand(
                        () =>
                        {
                            if (IsConnected)
                            {
                                //SendText = DateTime.Now.ToString("[yyyy-MM-dd HH:mm:ss.fff] Send：") + SendText;
                                SendStr(SendText);
                                ReceiveText += DateTime.Now.ToString("[yyyy-MM-dd HH:mm:ss.fff] Send：") + SendText + "\n";
                            }
                        }
                    );
                }
        ...
        [NonSerialized]
        private DelegateCommand _ClearSendTextCommand;
        [JsonIgnore]
        public DelegateCommand ClearSendTextCommand
        {
            get
            {
                if (_ClearSendTextCommand == null)
                {
                    _ClearSendTextCommand = new DelegateCommand(
                        () =>
                        {
                            SendText = "";
                        }
        ...
        [NonSerialized]
        private DelegateCommand _ClearReceiveTextCommand;
        [JsonIgnore]
        public DelegateCommand ClearReceiveTextCommand
        {
            get
            {
                if (_ClearReceiveTextCommand == null)
                {
                    _ClearReceiveTextCommand = new DelegateCommand(
                        () =>
                        {
                            ReceiveText = "";
                        }
        ...
        {
            Console.WriteLine($"监听到的数据：" + str);
            //Logger.AddLog($"[{Key}]接收数据:{str}");
            ReceiveText += DateTime.Now.ToString("[yyyy-MM-dd HH:mm:ss.fff] Receive：") + str + "\n";
        ...
        private void M_DMTcpClient_OnErrorMsg(string msg)
        {
            if (IsHasObjectConnected == true)
            {
                ...
                Console.WriteLine($"与服务器的连接断开  {RemoteIP}:{RemotePort}");
                //Logger.AddLog(
        ...
        private void M_DMTcpClient_OnStateInfo(string msg, SocketState state)
        {
            ...
            switch (state)
            {
                ...
                case SocketState.Connected:
                    ...
                    IsConnected = true;
                    Console.WriteLine($"已成功连接服务器  {RemoteIP}:{RemotePort}");
                    //Logger.AddLog(
        ...
        private void M_DMTcpServer_OnOfflineClient(Socket temp)
        {
            ...
            Console.WriteLine($"{temp.RemoteEndPoint.ToString()}客户端已断开连接");
            //Logger.AddLog(
        ...
        private void M_DMTcpServer_OnOnlineClient(Socket temp)
        {
            ...
            Console.WriteLine($"{temp.RemoteEndPoint.ToString()} 客户端已经连接");
            //Logger.AddLog(
        ...
        public bool SendStr(string str)
        {
            lock (this)
            {
                ...
                if (IsSendByHex == true)
                {
                    Console.WriteLine($"16进制数据:{HexTool.ToBytesFromHexString(str)}");
                ...
                if (flag == true)
                {
                    Console.WriteLine($"[{Key}]发送数据:{str}");
                    //Logger.AddLog($"[{Key}]发送数据:{str}");
                }
                else
                {
                    Console.WriteLine($"[{Key}]发送数据失败");
