        ...
        public static HObject MatToHObject(Mat mat)
        {
            ...
            try
            {
                if (mat.Channels() == 1)
                {
                    if (mat.Type() == MatType.CV_8UC1)
                    {
                        ...
                        try
                        {
                            ...
                            HOperatorSet.GenImage1(out hoImage, "byte", width, height, handle.AddrOfPinnedObject());
                        }
                        finally
                        {
                            if (handle.IsAllocated)
                    ...
                    else if (mat.Type() == MatType.CV_32FC1)
                    {
                        ...
                        {
                            handle = GCHandle.Alloc(data, GCHandleType.Pinned);
                            HOperatorSet.GenImage1(out hoImage, "real", width, height, handle.AddrOfPinnedObject());
                        }
                        finally
                        {
                            if (handle.IsAllocated)
                ...
                else if (mat.Channels() == 3 && mat.Type() == MatType.CV_8UC3)
                {
                    ...
                    {
                        if (redHandle.IsAllocated)
                        {
                            redHandle.Free();
                        }
                        if (greenHandle.IsAllocated)
                        {
                            greenHandle.Free();
                        }
                        if (blueHandle.IsAllocated)
                        {
                            blueHandle.Free();
                        }
