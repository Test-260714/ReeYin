...
namespace ReeYin_V.Core.Helper.ImageOP
{
    ...
    /// OpenCv绘制图像操作
    /// </summary>
    public static class OpenCvDrawing
    {
        /// <summary>
        /// 图片上绘制中文字符
        /// </summary>
        ...
        /// <returns></returns>
        public static Mat DrawChineseTextOnImage(Mat image, string text, System.Drawing.Point position, int fontSize, System.Drawing.Color textColor, string fontName = "宋体")
        {
            if (image.Empty())
            {
                Console.WriteLine("输入图像为空。");
                return image;
            }

            // 1. 将 OpenCvSharp.Mat 转换为 System.Drawing.Bitmap
            Bitmap bitmap = image.ToBitmap();

            // 2. 从 Bitmap 创建 Graphics 对象，用于 GDI+ 绘制
            using (Graphics g = Graphics.FromImage(bitmap))
            {
                ...
                // 3. 定义字体和画刷
                using (Font font = new Font(fontName, fontSize, System.Drawing.FontStyle.Bold))
                using (System.Drawing.Brush brush = new System.Drawing.SolidBrush(textColor))
                {
                    // 4. 绘制中文文本
            ...
            // 5. 将处理后的 System.Drawing.Bitmap 转换回 OpenCvSharp.Mat
            Mat outputImage = bitmap.ToMat();
